...
from ultralytics.nn.modules.head_se import Detect_SE
from ultralytics.nn.modules.layers.MobileNetv4 import MobileNetV4ConvSmall
...
from ultralytics.nn.modules import (AIFI, C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x,
                                    ...
                                    VanillaBlock, PatchMerging, PatchEmbed, SwinStage, GhostV2, DepthSepConv, SGBlock,
                                    CBRM, Shuffle_Block, stem, MBConvBlock, Conv_BN_HSwish, MobileNetV3_InvertedResidual,
                                    ConvNeXt_Stem, ConvNeXt_Block, ConvNeXt_Downsample, MV2Block, MobileViTBlock, ResNetLayer,
                                    StarBlock, MobileNetV3_BLOCK, InvertedBottleneck, mn_conv, SimAM, ECA, SpatialGroupEnhance, TripletAttention, CoordAtt, GAMAttention,
                                    ...
                                    C2f_CA, C2f_SE, C2f_ECA, C2f_CBAM, C2f_EffectiveSE, C2f_GlobalContext, C2f_GatherExcite, C2f_MHSA,
                                    C2f_SimAM, C2f_CoT, C2f_Double, C2f_SK, BiLevelRoutingAttention, C2f_Triplet, C2f_SpatialGroupEnhance, C2f_S2, C2f_NAM,
                                    C2f_ParNet, C2f_GAM, C2f_CrissCross, C2f_ParallelPolarized, C2f_SequentialPolarized,BiFPN_Add2, BiFPN_Add3,DRAMiTransformer,
                                    G_bneck,MEEM,CondConv,MogaBlock,SCSA,AxialImageTransformer,WTConv2d,PPA,SAFM
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect,Detect_SE,Detect_SEResNeXt, Segment)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect,Detect_SE,Detect_SEResNeXt, Segment, Pose)):
            s = 256  # 2x min stride
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def init_criterion(self, RTDETRDetectionLoss=None, ):
        """Compute the classification loss between predictions and true labels."""
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect,Detect_SE,Detect_SEResNeXt, Segment):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                ...
                BasicStage, PatchEmbed_FasterNet, PatchMerging_FasterNet,MobileNetV3_BLOCK, InvertedBottleneck, mn_conv,
                C2f_CA, C2f_SE, C2f_ECA, C2f_CBAM,C2f_EffectiveSE, C2f_GlobalContext, C2f_GatherExcite, C2f_MHSA,C2f_SimAM,
                 C2f_CoT, C2f_Double, C2f_SK, C2f_Triplet,
                 C2f_SpatialGroupEnhance, C2f_S2, C2f_NAM,
                 C2f_ParNet, C2f_GAM, C2f_CrissCross, C2f_ParallelPolarized, C2f_SequentialPolarized,PPA,
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3,C2f_CA, C2f_SE, C2f_ECA, C2f_CBAM,
                     C2f_EffectiveSE, C2f_GlobalContext, C2f_GatherExcite, C2f_MHSA,C2f_SimAM, C2f_CoT, C2f_Double, C2f_SK,C2f_Triplet,
                     C2f_SpatialGroupEnhance, C2f_S2, C2f_NAM,C2f_ParNet, C2f_GAM, C2f_CrissCross, C2f_ParallelPolarized, C2f_SequentialPolarized,
        ...
        elif m is CondConv:  # DynamicConv
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]

        elif m in [MEEM]:  # C1 C2 args
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
        ...
        elif m in (Detect, Segment, Pose, RTDETRDecoder,Detect_SE,Detect_SEResNeXt):
        ...
        if m in [MobileNetV4ConvSmall]:
                m_ = m(args[2])  # module
                t = "ultralytics.nn.modules.layers.MobileNetv4"
        else:
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Detect,Detect_SE,Detect_SEResNeXt)):
                return 'detect'
